...
def extract_banner_mood_color(art_url):
    ...
    try:
        with Image.open(img_path) as img:
            ...
            # Boost the primary vibrancy vectors
            r = min(255, r + 80)
            g = max(0, g - 40)
            b = max(0, b - 100)
            ...
            # Normalize to maximum possible brightness ceiling (255) instead of 60
            luminance = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255
            
            if luminance > 0.01:
                scale = 255 / (luminance * 255)
...
def main():
    ...
    # Uncapped default starting color base
        ...
        while True:
            ...
            if current_time - last_spotify_check > spotify_check_interval:
                ...
                if current_url and current_url != last_url:
                    ...
                    base_r, base_g, base_b = extract_banner_mood_color(current_url)
                    print(f"New Album Art! Base RGB: ({base_r}, {base_g}, {base_b})")
            ...
            if ready:
                ...
                if line:
                    try:
                        ...
                        if bars:
                            avg_intensity = sum(bars[:2]) / 2
                            #avg_intensity = sum(bars[:2]) / 2 (BASS ONLY)
                            #avg_intensity = sum(bars) / len(bars) (LYRICS ONLY)
                            # Scaling logic: Maps volume to a 10% min / 100% max brightness factor
                            brightness_multiplier = 0.1 + (avg_intensity / 100.0) * 0.9
                            
                            # Mix uncapped base color with audio scaling
